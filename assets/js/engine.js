...
window.NS_ENGINE = (function (D) {
  ...
  function genLitres(seed) {
    ...
    const blurbs = [
      // 1. эмоционально-вдохновляющая
      `«${T}» — книга, которая меняет то, как вы смотрите на ${main}${also}. ` +
      `Она возвращает опору, когда её не хватает, и мягко ведёт к переменам. ` +
      `Здесь нет давления и сухой теории — только честные слова, живые примеры и вера в то, что у вас получится. ` +
      `Начните читать — и почувствуйте, как ${main} перестаёт пугать и становится вашей силой.`,
      // 2. деловая экспертная
      `Практическое руководство по теме «${main}»${also}. ` +
      `Автор собрал рабочую систему из проверенных подходов и разложил её по шагам — без воды. ` +
      `Понятная структура, конкретные инструменты и примеры, которые можно применить сразу. ` +
      `Книга для тех, кто хочет результат, а не мотивационные лозунги.`,
      // 3. интригующая
      `Что если всё, что вы знали про ${main}, — только половина правды? ` +
      `«${T}» переворачивает привычный взгляд и показывает то, что обычно остаётся за кадром. ` +
      `История, идеи и приёмы, от которых сложно оторваться. ` +
      `Откройте первую главу — и вы уже не сможете закрыть книгу.`,
      // 4. тёплая разговорная
      `Эта книга — честный разговор о ${main}${also}. ` +
      `Я написал её для тех, кто устал притворяться, что «всё в порядке», и хочет разобраться по-настоящему. ` +
      `Без осуждения, простыми словами, как за чашкой чая. ` +
      `Если вам близка эта тема — устраивайтесь поудобнее, эта книга для вас.`,
      // 5. структурная / выгоды
      `О чём книга «${T}»: про ${main}${also} — понятно и по делу. ` +
      `Что вы получите: ясную картину темы, пошаговый план, конкретные техники и примеры из жизни. ` +
      `Для кого: для всех, кто хочет двигаться вперёд без выгорания и лишней теории. ` +
      `Итог: прочитали — применили — увидели результат. Начните сегодня.`
    ];
    return [
  ...
  function genPodcast(seed) {
    ...
    const also = cs[1] ? ' и ' + cs[1] : '';
    const descs = [
      // 1. энергичная вовлекающая
      `Погнали! В этом выпуске разбираем ${main}${also} — быстро, честно и по делу. ` +
      `Никакой воды: только рабочие идеи и примеры, которые можно применить уже сегодня. ` +
      `Слушай до конца, лови инсайты и подписывайся, чтобы не пропустить новые выпуски.`,
      // 2. экспертно-деловая
      `Тема выпуска — ${main}${also}. Разбираем системно: что важно знать, какие ошибки мешают и что реально работает. ` +
      `Чёткие выводы, конкретные шаги и проверенные подходы без лишней теории. ` +
      `Подписывайтесь на подкаст, чтобы получать разборы регулярно.`,
      // 3. интригующая
      `Что скрывается за темой «${main}»? ` +
      `В этом выпуске мы задаём неудобные вопросы и ищем на них честные ответы. ` +
      `Будет неожиданно. Включай — и слушай до последней минуты.`,
      // 4. тёплая личная
      `Сегодня хочу поговорить с тобой про ${main}${also}. ` +
      `Без масок и умных слов — просто честно делюсь тем, что поняла сама. ` +
      `Если тебе близка эта тема, завари чай, устройся поудобнее и оставайся до конца. Обнимаю.`,
      // 5. структурная
      `Выпуск про ${main}${also}. Что внутри: главное по теме, разбор частых ошибок, конкретные шаги и выводы, которые заберёшь с собой. ` +
      `Для кого: для всех, кому важна эта тема. ` +
      `Слушай, сохраняй и делись с теми, кому будет полезно. Подписка помогает подкасту расти.`
    ];
  ...
  const AI_SPEC = {
    ...
    litres: 'Это книга (тебе дают название, концепцию, текст или аннотацию). Верни 4 группы: ' +
      ...
      '"Аннотации — 5 версий" — ровно 5 разных готовых вариантов аннотации, каждый 700–1100 знаков, под поиск ЛитРес и естественно насыщенный ключевыми словами по теме книги. ' +
    ...
    podcast: 'Тебе дают текущее описание выпуска подкаста или его транскрибацию. ' +
      'Верни 4 группы: ' +
      '"Описания выпуска — 5 версий" — ровно 5 разных готовых вариантов описания для площадок (Apple Podcasts, Spotify, Яндекс Музыка), каждый 600–1200 знаков, ' +
